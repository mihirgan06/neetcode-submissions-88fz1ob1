...
class Solution:
    def kthSmallest(self, root: Optional[TreeNode], k: int) -> int:
        '''
            ...
            inorder = 2, 3, 4, 5, 7, 8
            left node right
            Process in order traversal as we now traverse the BSt in order

            then increment count as we process out actual node
            if count is equal to k then return node.val
            call dfs on right side
