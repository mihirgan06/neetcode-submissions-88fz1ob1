class Solution {
    ...
    * When you see a problem with Nodes AND edges [u,v] --> Adjacency list
    When input is int n (number of nodes) and int[][] edges (list of edges)
    you use adj.getnode to return list of neithbors

    So n nodes, edges --> adj list ALWAYS

    For neighbors use adj.get(node)
